...
public class CaseClientDetailsController {

	private static final String SUCCESS_CODE_200 = "SUCCESS_CODE_200";

	private static final Logger logger = LoggerFactory.getLogger(CaseClientDetailsController.class);

	@Autowired
	private CaseClientDetailsService caseClientDetailsService;

	@ApiOperation(value = "This API is used for fetching all client related details for given case id", response = ResponseStatusPOJO.class)
	@GetMapping(path = "case-client-details/{caseId}", produces = "application/json")
	public ResponseEntity<ResponseStatusPOJO> getCaseClientDetails(@PathVariable(value = "caseId") Long caseId) {
		logger.info("Case id to find : {}", caseId);
		try {
			return new ResponseEntity<>(new ResponseStatusPOJO(true, "Record found", SUCCESS_CODE_200,
					caseClientDetailsService.getCaseClientDetailsByCaseDetailsId(caseId)), HttpStatus.OK);
